...
// Appelé depuis app.js pour enregistrer le originalSetItem
function enregistrerSetItem(fn) {
  _originalSetItem = fn;
}
...
const CLES_SYNC = [
  ...
  'au-parties',
  'golf-scores',
  'notes-joueur1',  'notes-joueur2',  'notes-joueur3',
  'notes-joueur4',  'notes-joueur5',  'notes-joueur6',
  'notes-joueur7',  'notes-joueur8',  'notes-joueur9',
  'notes-joueur10', 'notes-joueur11', 'notes-joueur12',
  'pseudos-joueur1',  'pseudos-joueur2',  'pseudos-joueur3',
  'pseudos-joueur4',  'pseudos-joueur5',  'pseudos-joueur6',
  'pseudos-joueur7',  'pseudos-joueur8',  'pseudos-joueur9',
  'pseudos-joueur10', 'pseudos-joueur11', 'pseudos-joueur12',
];
  ...
  for (const cle of CLES_SYNC) {
    try {
      ...
      if (data && data.valeur !== undefined) {
        // Utiliser _originalSetItem si disponible, sinon localStorage natif
        const setter = _originalSetItem || localStorage.setItem.bind(localStorage);
        setter(cle, JSON.stringify(data.valeur));
        console.log('📥 Chargé :', cle);
      }
    } catch (e) {
  ...
  CLES_SYNC.forEach(function(cle) {
    fbWatch('lan-data', cle, function(data) {
      ...
      if (valeurActuelle !== nouvelleValeur) {
        // Utiliser _originalSetItem si disponible
        const setter = _originalSetItem || localStorage.setItem.bind(localStorage);
        setter(cle, nouvelleValeur);
...
export { chargerDepuisFirebase, activerTempsReel, syncVersFirebase, CLES_SYNC, enregistrerSetItem };